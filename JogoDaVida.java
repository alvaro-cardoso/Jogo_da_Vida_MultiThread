public class JogoDaVida{
    ...
    public static void main(String[] args){
        ...
        RunThreadJogoDaVida[] rt = new RunThreadJogoDaVida[noThreads];
        int totalAlive;
        for(int g = 0; g < T; g++){  
            for(i = 0; i < noThreads; i++){
                rt[i] = new RunThreadJogoDaVida(i, noThreads, grid);
        ...
        System.out.println("Took " + calcTime + "ms.");
    }
}

class RunThreadJogoDaVida implements Runnable{
    private int id;
    private int N;
    ...
    public RunThreadJogoDaVida(int tId, int tNoThreads, int[][] tGrid){
        id = tId;
        grid = tGrid;
